...
# atomselect macro lipid {resname DLPE DMPC DPPC GPC LPPC PALM PC PGCL POPC POPE POPG DOPE DOPS DOPC POUE POUC POUN POUG PXPE PXPC PXPG DHUC DMUC}

proc rotlipids {mol dist} {
  foreach resname {DLPE DMPC DPPC GPC LPPC PALM PC PGCL POPC POPE POPG DOPE DOPS DOPC POUE POUC POUN POUG PXPE PXPC PXPG DHUC DMUC} {
    set lipidresname($resname) 1
  }
  set all [atomselect $mol "not hydrogen"]
  display update ui
  foreach {ca cb} [measure contacts $dist $all] {}
  ...
  while { [llength $cc] } {
    ...
    set cxyz {}
    set i 0
    foreach rec [$csel get {index resname residue x y z}] {
      foreach {index resname residue x y z} $rec {}
      set cidx($index) [list $i [info exists lipidresname($resname)] $residue]
    ...
    set dotcounter 0
    foreach a1 $ca a2 $cb {
      foreach {a1 isr1lipid r1} $cidx($a1) {}
      foreach {a2 isr2lipid r2} $cidx($a2) {}
      if { $r1 == $r2 } { incr samerescounter; continue }
      if { ! ( $isr1lipid || $isr2lipid ) } { incr samerescounter; continue }
