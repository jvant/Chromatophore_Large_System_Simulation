...
#calculate weighted average of lipid types
	...
	for {set i 0} {$i < $len} {set i [expr {$i + 2}]} {
		...
		switch $name {
                        ...
                        #decreasing the APL by 5% so that the lipids will be more closely packed
			POPE { set apl [expr {(63.0*0.95*$perc) + $apl}] }
			POPC { set apl [expr {(68.3*0.95*$perc) + $apl}] }
			POPS { set apl [expr {(62.0*0.95*$perc) + $apl}] }
			POPG { set apl [expr {(62.0*0.95*$perc) + $apl}] }
		}
	}
	return $apl
}
